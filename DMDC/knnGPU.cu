...
// Install CUDA
// Add necessary path variables

// Next compile with:
// nvcc -m 64 -I "/Applications/MATLAB_R2011b.app/extern/include/" --cuda knnGPU.cu --output-file knnGPU.cpp

// Then mex in matlab with
// mex -I/usr/local/cuda/include -L/usr/local/cuda/lib -lcudart -lcufft -lcublas -lcuda knnGPU.cpp
...
// Constants used by the program MAX_TEXTURE_WIDTH 65536?
//#define MAX_PITCH_VALUE_IN_BYTES       262144
#define MAX_PITCH_VALUE_IN_BYTES       2147483647
#define MAX_TEXTURE_WIDTH_IN_BYTES     65535
...
__global__ void cuAddQNormAndSqrt(float *dist, int width, int pitch, float *q, int k){
    ...
    if (xIndex<width && yIndex<k)
        dist[yIndex*pitch + xIndex] = sqrt(abs(dist[yIndex*pitch + xIndex] + q[xIndex]));
}
...
/**
  ...
  *
  */
void knn(float* ref_host, int ref_width, float* query_host, int query_width, int height, int k, float* dist_host, int* ind_host, int gpuID){
    ...
    unsigned int       actual_nb_query_width;
    size_t memory_total;
    size_t memory_free;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    // Call KNN CUDA
    knn(ref, ref_width, query, query_width, ref_height, k, dist, ind, gpuID);
...
int main(void){
	...
	int    ref_nb     = 65096;   // Reference point number, max=65535
	int    query_nb   = 65096;   // Query point number,     max=65535
